        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <h1 className="text-3xl md:text-4xl font-black text-charcoal flex items-center gap-4 tracking-tight">
              <div className="p-2.5 bg-vision-green/10 rounded-2xl shadow-sm border border-vision-green/10">
                <SettingsIcon className="w-8 h-8 text-vision-green" />
              </div>
              System Settings
            </h1>
            ...
            <div className="p-8 border-b border-light-gray bg-gray-50/40 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                <h2 className="text-xl font-black text-charcoal flex items-center gap-2.5 uppercase tracking-wider">
                  <Users className="w-5 h-5 text-solar-orange" />
                  Team Management
                </h2>
              ...
              {!loading && (
                <div className="bg-white px-4 py-2 rounded-xl border border-light-gray shadow-sm">
                  <span className="text-xs font-black text-vision-green uppercase tracking-widest">{users.length} Active Members</span>
                </div>
              )}
            </div>
            ...
            {loading ? (
              <div className="p-24 flex flex-col items-center justify-center gap-4">
                <div className="relative">
                   <Loader2 className="w-12 h-12 text-vision-green animate-spin" />
                   <div className="absolute inset-0 flex items-center justify-center">
                     <Users className="w-4 h-4 text-vision-green/40" />
                   </div>
                </div>
                    ...
                    <div key={user.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:bg-off-white/40 transition-all duration-300 group relative">
                      <div className="flex items-center gap-5">
                        <Avatar className="w-14 h-14 border-4 border-white shadow-md ring-1 ring-light-gray/50 group-hover:scale-105 transition-transform duration-500">
                          <AvatarFallback className="bg-gradient-to-br from-vision-green to-green-dark text-white font-black text-xl">
                            {user.full_name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'U'}
                          </AvatarFallback>
                        </Avatar>
                        <div className="space-y-1">
                          <p className="font-black text-charcoal text-lg group-hover:text-vision-green transition-colors leading-none tracking-tight">{user.full_name}</p>
                      ...
                      <div className="flex items-center gap-4 self-end sm:self-auto">
                        <div className="text-right hidden md:block">
                           <div className="flex items-center justify-end gap-1.5 mb-0.5">
                             <Shield className="w-3 h-3 text-vision-green" />
                             <span className="text-[10px] font-black text-vision-green uppercase tracking-widest">{user.role}</span>
                           </div>
